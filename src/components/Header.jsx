...
import { Link } from "react-router-dom"; // დავამატეთ Link ნავიგაციისთვის
import { useCart } from "./context/CartContext";
import CartModal from "./CartModal";

export default function Header() {
    // 1. მოდალის გახსნა-დაკეტვის სთეითი
    const [isCartOpen, setIsCartOpen] = useState(false);

    // 2. ამოვიღოთ კალათის ნივთები კონტექსტიდან
    const { cartItems } = useCart();

    // 3. დავთვალოთ ნივთების ჯამური რაოდენობა ბეიჯისთვის
    const totalItems = cartItems.reduce((acc, item) => acc + item.quantity, 0);

    // ფუნქცია მოდალის გადასართავად
        ...
        <>
            <header className="header">
                ...
                <nav className="nav">
                    ...
                    <Link className="pages" to="/headphones">Headphones</Link>
                    <Link className="pages" to="/speakers">Speakers</Link>
                    <Link className="pages" to="/earphones">Earphones</Link>
                </nav>
                ...
                >
                    ...
                    {/* Badge - ნარინჯისფერი წრე რაოდენობით */}
                    {totalItems > 0 && (
                        <span style={{
                            position: "absolute",
                            top: "-10px",
